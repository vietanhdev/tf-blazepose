...
@tf.function
def calc_pck(batch_keypoints_true, batch_keypoints_pred, ref_point_pair=(2, 4), thresh=0.5):
    ...
    wrong_matrix = tf.cast(error, tf.float32) > (tf.cast(ref_distance, tf.float32) * thresh)
...
def get_pck_metric(ref_point_pair=(2, 4), thresh=0.2):
    ...
    class PCK(tf.keras.metrics.Metric):
        ...
        def __init__(self, name='pck', ref_point_pair=ref_point_pair, thresh=thresh, **kwargs):
